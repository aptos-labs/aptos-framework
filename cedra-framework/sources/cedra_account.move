module cedra_framework::cedra_account {
    ...
    fun test_primary_fungible_store_address(
        ...
        let cedra_metadata = object::address_to_object<Metadata>(@cedra_fungible_asset);
        let user_addr = signer::address_of(user);
        assert!(primary_fungible_store_address(user_addr) == primary_fungible_store::primary_store_address(user_addr, cedra_metadata), 1);

        ensure_primary_fungible_store_exists(user_addr);
        assert!(primary_fungible_store::primary_store_exists(user_addr, cedra_metadata), 2);
    }
}
