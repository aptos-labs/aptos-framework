spec cedra_framework::transaction_validation {
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        let maybe_cedra_supply = global<CoinInfo<CedraCoin>>(cedra_addr).supply;
        let total_supply_enabled = option::spec_is_some(maybe_cedra_supply);
        let cedra_supply = option::spec_borrow(maybe_cedra_supply);
        let cedra_supply_value = optional_aggregator::optional_aggregator_value(cedra_supply);
        let post post_maybe_cedra_supply = global<CoinInfo<CedraCoin>>(cedra_addr).supply;
        ...
        aborts_if amount_to_burn > 0 && !exists<CoinInfo<CedraCoin>>(cedra_addr);
        aborts_if amount_to_burn > 0 && total_supply_enabled && cedra_supply_value < amount_to_burn;
        ensures total_supply_enabled ==> cedra_supply_value - amount_to_burn == post_cedra_supply_value;
